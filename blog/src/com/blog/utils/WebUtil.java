...
public class WebUtil {
    ...
    //去除html 
    public static List<ArticleInfo> removeArtilceHtml(List<ArticleInfo> articles)
    {
       List<ArticleInfo> newarticle = null;
        if(articles != null && articles.size() > 0)
        {
            newarticle = new ArrayList<ArticleInfo>();
            for(int i = 0 ; i < articles.size(); i++)
            {
                String artc = articles.get(i).getDescription().replaceAll("<.*?>", "").replace("&nbsp;", "");
                if(artc != null && artc.length() > 300)
                {
                    artc = artc.substring(0, 300) + " ... ";
                }
                articles.get(i).setDescription(artc);
